...
def create_directories_and_copy_certificates():
    ...
    run_command(f'docker exec {container_name} cp /path/to/csr.crt /usr/local/apache2/conf/ssl/server.crt')
    run_command(f'docker exec {container_name} cp /path/to/csr.key /usr/local/apache2/conf/ssl/server.key')
